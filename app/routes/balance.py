...
async def get_user_balance(user_id: str, session=Depends(get_session))->Balance:
...
async def increase_user_balance(user_id:str, credits: float, session=Depends(get_session))->None:
    BalanceService.increase_user_balance(uuid.UUID(user_id), credits, session)
